...
def generate_tests(f, rs1_val, rs2_val, vsew, lmul):
    ...
    for i in range(2):
        if 2 * emul <= 8 and 2 + 2 * emul <= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG2_OP( "+str(n)+",  %s.v, " %instr+" 16 "+", "+"0 + tdat"+" );", file=f)
            n += 1
            print("  TEST_VLSEG2_OP( "+str(n)+",  %s.v, " %instr+" 16 "+", "+"2 + tdat"+" );", file=f)
        if 3 * emul <= 8 and 8 + 3 * emul <= 32 and 8 + 3 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG3_OP( "+str(n)+",  %s.v, " %instr1+" 16 "+", "+"0 + tdat"+" );", file=f)
        if 4 * emul <= 8 and 8 + 4 * emul <= 32 and 8 + 4 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG4_OP( "+str(n)+",  %s.v, " %instr2+" 16 "+", "+"8 + tdat"+" );", file=f)
        if 5 * emul <= 8 and 8 + 5 * emul <= 32 and 8 + 5 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG5_OP( "+str(n)+",  %s.v, " %instr3+" 16 "+", "+"-12 + tdat4"+" );", file=f)
        if 6 * emul <= 8 and 8 + 6 * emul <= 32 and 8 + 6 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG6_OP( "+str(n)+",  %s.v, " %instr4+" 16 "+", "+"0 + tdat"+" );", file=f)
        if 7 * emul <= 8 and 8 + 7 * emul <= 32 and 8 + 7 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG7_OP( "+str(n)+",  %s.v, " %instr5+" 16 "+", "+"0 + tdat"+" );", file=f)
        if 8 * emul <= 8 and 8 + 8 * emul <= 32 and 8 + 8 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("  TEST_VLSEG8_OP( "+str(n)+",  %s.v, " %instr6+" 16 "+", "+"0 + tdat"+" );", file=f)
        

    if 2 * emul <= 8 and 2 + 2 * emul <= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
        for i in range(100):     
            k = i%30+1
            if k != 8 and k != 16 and k % emul == 0 and k % lmul == 0 and k % lmul == 0 and k + 2 * emul <= 32 and k!= 12 and k != 20 and k !=24: # (insn.rd() + nf * emul) <= NVPR
                n+=1
                print("  TEST_VLSEG1_OP_rd%d( "%k+str(n)+",  %s.v, "%instr+" 16 "+", "+"0 + tdat"+" );",file=f)
            
            k = i%30+2
            if(k == 31  or k == 12 or k == 20 or k == 24):
                continue;
            n +=1
            print("  TEST_VLSEG1_OP_1%d( "%k+str(n)+",  %s.v, "%instr+" 16 "+", "+"4 + tdat"+" );",file=f)
    return n
