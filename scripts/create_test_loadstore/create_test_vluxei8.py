...
def generate_tests(f, rs1_val, rs2_val, vsew, lmul):
    ...
    for i in range(2):
        ...
        print("  TEST_VLXEI_OP( "+str(n)+",  %s.v, " %
              instr+" 8 "+", "+("0x"+tdats[int(-vsew/4):])+", "+("0x"+tdats[2*int(-vsew/4):int(-vsew/4)])+", "+"0 + tdat"+", "+"idx8dat"+" );", file=f)
        n += 1
        print("  TEST_VLXEI_OP( "+str(n)+",  %s.v, " %
              instr+" 8 "+", "+("0x"+tdats[int(-vsew/4)-32:-32])+", "+("0x"+tdats[2*int(-vsew/4)-32:int(-vsew/4)-32])+", "+"0 + tdat5"+", "+"idx8dat"+" );", file=f)
    ...
    for i in range(100):     
        ...
        if k % emul == 0 and k % lmul ==0 and k not in [31, 8, 16] and not is_overlap(k, lmul, 8, emul):
            n+=1
            print("  TEST_VLXEI_OP_rd%d( "%k+str(n)+",  %s.v, "%instr+" 8 "+", "+("0x"+tdats[int(-vsew/4)-16:-16])+", "+("0x"+tdats[2*int(-vsew/4)-16:int(-vsew/4)-16])+" , "+"-8 + tdat5"+", "+"idx8dat"+" );",file=f)
        
        k = i%30+2
        if k % emul == 0 and k not in [31, 8, 16] and not is_overlap(k, lmul, 8, emul):
            n +=1
            print("  TEST_VLXEI_OP_1%d( "%k+str(n)+",  %s.v, "%instr+" 8 "+", "+("0x"+tdats[int(-vsew/4):])+", "+("0x"+tdats[2*int(-vsew/4):int(-vsew/4)])+" , "+"0 + tdat"+", "+"idx8dat"+" );",file=f)
