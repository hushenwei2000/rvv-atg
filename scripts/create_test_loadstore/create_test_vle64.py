...
def generate_tests(f, rs1_val, rs2_val, vsew, lmul):
    ...
    for i in range(2):
        ...
        n += 1
        print("  TEST_VLE_OP( "+str(n)+",  %s.v, " %
              instr+" 64 "+", "+"0xf00ff00f0ff00ff0"+", "+"0xff00ff0000ff00ff"+" , "+"-4 + tdat4"+" );", file=f)
        n += 1
    ...
    for i in range(100):     
        ...
        if( k % lmul == 0 and k % emul == 0 and k != 12 and k != 20 and k != 24):
            print("  TEST_VLE_OP_rd%d( "%k+str(n)+",  %s.v, "%instr+" 64 "+", "+"0xf00ff00f0ff00ff0"+", "+"0xff00ff0000ff00ff"+" , "+"-4 + tdat4"+");",file=f)
...
def create_first_test_vle64(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
