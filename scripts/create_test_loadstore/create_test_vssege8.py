...
def generate_tests(f, rs1_val, rs2_val, vsew, lmul):
    ...
    for i in range(2):
        if 2 * emul <= 8 and 2 + 2 * emul <= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSEG2_OP( "+str(n)+", %s.v, %s.v, "%(instr1,instr)+"8"+", "+"0 + tdat"+", rd_origin_data);", file=f)
            n += 1
            print("   TEST_VSSEG2_OP( "+str(n)+", %s.v, %s.v, "%(instr1,instr)+"8"+", "+"1 + tdat"+", rd_origin_data);", file=f)
            n += 1
    ...
    if 2 * emul <= 8 and 2 + 2 * emul <= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
        for i in range(100):     
            ...
            if k == 8 or k == 16 or k == 24: # (insn.rd() + nf * emul) <= NVPR
                n+=1
                print("  TEST_VSSEG1_OP_rd%d( "%k+str(n)+", %s.v, %s.v, "%(instr1,instr)+"8"+", "+"0 + tdat"+" , rd_origin_data);",file=f)
        
            k = i%30+2
            if(k == 31 or k == 12 or k == 20 or k == 24):
                continue;
            n +=1
            print("   TEST_VSSEG1_OP_1%d( "%k+str(n)+", %s.v, %s.v, "%(instr1,instr)+"8"+", "+"-8 + tdat8"+" , rd_origin_data);",file=f)
    return n
