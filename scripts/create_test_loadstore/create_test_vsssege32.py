...
def generate_tests(f, rs1_val, rs2_val, vsew, lmul):
    ...
    for i in range(2):
        if 2 * emul <= 8 and 2 + 2 * emul <= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            ...
            n += 1
            print("   TEST_VSSSEG2_OP( "+str(n)+", %s.v, %s.v, "%(instr1,instr)+"32"+", "+"0"+", "+"4 + tdat"+", rd_origin_data);", file=f)
            n += 1
            print("   TEST_VSSSEG2_OP( "+str(n)+", %s.v, %s.v, "%(instr1,instr)+"32"+", "+"4"+", "+"0 + tdat"+", rd_origin_data);", file=f)
            n += 1
            print("   TEST_VSSSEG2_OP( "+str(n)+", %s.v, %s.v, "%(instr1,instr)+"32"+", "+"4"+", "+"4 + tdat"+", rd_origin_data);", file=f)
        if 3 * emul <= 8 and 8 + 3 * emul <= 32 and 8 + 3 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSSEG3_OP( "+str(n)+", %s.v, %s.v, "%(instr2l,instr2)+"32"+", "+"0"+", "+"0 + tdat"+", rd_origin_data);", file=f)
        if 4 * emul <= 8 and 8 + 4 * emul <= 32 and 8 + 4 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSSEG4_OP( "+str(n)+", %s.v, %s.v, "%(instr3l,instr3)+"32"+", "+"0"+", "+"0 + tdat"+", rd_origin_data);", file=f)
        if 5 * emul <= 8 and 8 + 5 * emul <= 32 and 8 + 5 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSSEG5_OP( "+str(n)+", %s.v, %s.v, "%(instr4l,instr4)+"32"+", "+"0"+", "+"0 + tdat"+", rd_origin_data);", file=f)
        if 6 * emul <= 8 and 8 + 6 * emul <= 32 and 8 + 6 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSSEG6_OP( "+str(n)+", %s.v, %s.v, "%(instr5l,instr5)+"32"+", "+"0"+", "+"0 + tdat"+", rd_origin_data);", file=f)
        if 7 * emul <= 8 and 8 + 7 * emul <= 32 and 8 + 7 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSSEG7_OP( "+str(n)+", %s.v, %s.v, "%(instr6l,instr6)+"32"+", "+"0"+", "+"0 + tdat"+", rd_origin_data);", file=f)
        if 8 * emul <= 8 and 8 + 8 * emul <= 32 and 8 + 8 * lmul<= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
            n += 1
            print("   TEST_VSSSEG8_OP( "+str(n)+", %s.v, %s.v, "%(instr7l,instr7)+"32"+", "+"0"+", "+"0 + tdat"+", rd_origin_data);", file=f)
    ...
    if 2 * emul <= 8 and 2 + 2 * emul <= 32: # (nf * emul) <= (NVPR / 4) &&  (insn.rd() + nf * emul) <= NVPR);
        for i in range(100):     
            ...
            if k == 8 or k == 16 or k == 24: # (insn.rd() + nf * emul) <= NVPR
                n+=1
                print("   TEST_VSSSEG1_OP_rd%d( "%k+str(n)+", %s.v, %s.v, "%(instr1,instr)+"32"+", "+"0"+",  "+"0 + tdat"+", rd_origin_data);",file=f)
        
            k = i%30+2
            if(k == 31 or k == 12 or k == 20 or k == 24  or k == 29 or k == 30):
                continue;
            n +=1
            print("    TEST_VSSSEG1_OP_1%d( "%k+str(n)+", %s.v, %s.v, "%(instr1,instr)+"32"+", "+"32"+",  "+"0 + tdat"+", rd_origin_data);",file=f)
    return n
