...
def generate_tests(f, rs1_val, rs2_val, vsew, lmul):
    ...
    for i in range(2):
        ...
        if 8 * lmul <= 8 and 16 + 8 * lmul <= 32:
            ...
            n += 1
            print("  TEST_VLXSEG8_OP( "+str(n)+",  %s.v, " %instr6+" 8 "+", "+"4096 + tdat"+", "+"idx8dat"+" );", file=f)
    ...
    if 2 * lmul <= 8 and 2 + 2 * lmul <= 32: # (nf * lmul) <= (NVPR / 4) &&  (insn.rd() + nf * lmul) <= NVPR);
        for i in range(100):     
            ...
            if k != 8 and k != 16 and k % lmul == 0 and k % emul == 0 and k % lmul == 0 and k + 2 * lmul <= 32 and not is_overlap(k, lmul*2, 8, emul) and k!= 12 and k != 20 and k !=24:
                n+=1
                print("   TEST_VLXSEG1_OP_rd%d( "%k+str(n)+",  %s.v, "%instr+"8"+", "+"0 + tdat"+", "+"idx8dat"+");",file=f)
            
            k = i%30+2
            if( k == 12 or k == 20 or k == 24):
                continue;
            n +=1
            print("  TEST_VLXSEG1_OP_1%d( "%k+str(n)+",  %s.v, "%instr+"8"+", "+"0 + tdat"+", "+"idx8dat"+" );",file=f)
