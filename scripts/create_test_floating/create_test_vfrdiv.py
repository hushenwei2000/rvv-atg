...
def generate_tests(f, rs1_val, rs2_val):
    n = 1
    print("  #-------------------------------------------------------------",file=f)
    print("  # VFR Tests",file=f)
    print("  #-------------------------------------------------------------",file=f)
    print("  RVTEST_SIGBASE( x20,signature_x20_1)",file=f)
    for i in range(len(rs1_val)):
        print("TEST_FP_VF_OP( %d, vfrdiv.vf, 0xff100,               5201314,        %s,        %s );"%(n, rs1_val[i], rs2_val[i]), file=f)
        n += 1

    print("  #-------------------------------------------------------------",file=f)
    print("  # VFR Tests (different register)",file=f)
    print("  #-------------------------------------------------------------",file=f)
    ...
    for i in range(len(rs1_val)):     
        ...
        if k == 1:
            continue       
        print("  TEST_FP_VF_OP_rd_%d( "%k+str(n)+",  vfrdiv.vf, 0xff100, " +"5201314"+", "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
        n += 1
        
        k = i%31+1
        if k == 2:
            continue        
        print("  TEST_FP_VF_OP_rs1_%d( "%k+str(n)+",  vfrdiv.vf, 0xff100, " +"5201314"+", "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
        n += 1
...
def create_empty_test_vfrdiv(xlen, vlen, vsew, lmul, vta, vma, output_dir):
...
def create_first_test_vfrdiv(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
