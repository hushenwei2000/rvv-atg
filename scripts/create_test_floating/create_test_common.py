    ...
    if test_vv:
        for n in range(1, 32):
            ...
            TEST_CASE_LOOP_FP( testnum, v%d, flags, result, v8, \\\n\
                ...
                vle%d.v v%d, (x7); \\\n\
                inst v%d, v%d, v%d; \\\n\
            )" % (n, rd, vsew, rs2, vsew, n, rd, rs2, n), file=f)
        for n in range(1, 32):
            if n % lmul != 0:
                continue
            ...
            TEST_CASE_LOOP_FP( testnum, v%d, flags, result, v8, \\\n\
                ...
                vle%d.v v%d, (x7); \\\n\
                inst v%d, v%d, v%d; \\\n\
            )" % (n, n, vsew, rs2, vsew, rs1, n, rs2, rs1), file=f)
    if test_vf:
        for n in range(1,32):
            if n == 2 or n % lmul != 0:
                continue
    ...
    if test_vv:
        for n in range(1, 32):
            ...
            TEST_CASE_FP( testnum, v%d, flags, result, val1, val2, \\\n\
                ...
                fl%s f2, %d(a0); \\\n\
                inst v%d, v%d, v%d; \\\n\
            )" % (n, rd, ('d' if vsew == 64 else 'w'), ('d' if vsew == 64 else 'w'), (8 if vsew == 64 else 4), rs2,  n, ('d' if vsew == 64 else 'w'), (16 if vsew == 64 else 8), rd, rs2, n), file=f)
        for n in range(1, 32):
            if n % lmul != 0:
                continue
            rs1, rs2 = valid_aligned_regs(n)
            ...
            TEST_CASE_FP( testnum, v%d, flags, result, val1, val2, \\\n\
                ...
                fl%s f2, %d(a0); \\\n\
                inst v%d, v%d, v%d; \\\n\
            )" % (n,  n, ('d' if vsew == 64 else 'w'), ('d' if vsew == 64 else 'w'), (8 if vsew == 64 else 4), rs2, rs1, ('d' if vsew == 64 else 'w'), (16 if vsew == 64 else 8), n, rs2, rs1), file=f)
    if test_vf:
        for n in range(1,32):
            if n == 2 or n % lmul != 0:
                continue
