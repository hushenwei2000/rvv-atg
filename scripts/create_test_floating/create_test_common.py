...
def generate_tests(instr, f, vsew, lmul, test_vv = True, test_vf = True, test_rv = False):
    ...
    if test_vv:
        ...
        for i in range(len(rs1_val)):
            ...
            if k % lmul != 0:
                continue
            print("  TEST_FP_VV_OP_rd%d( "%k+str(n)+",  %s.vv, 0xff100, "%instr+"5201314"+", "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
            n += 1
            print("  TEST_FP_VV_OP_1%d( "%k+str(n)+",  %s.vv, 0xff100, "%instr+"5201314"+", "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
            n += 1
