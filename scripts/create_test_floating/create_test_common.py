    ...
    if test_vv:
        for n in range(1, 32):
            ...
            TEST_CASE_FP( testnum, v%d, flags, result, val1, val2, \\\n\
                ...
                fl%s f2, 8(a0); \\\n\
                inst v%d, v%d, v%d; \\\n\
            )" % (n, rd, ('d' if vsew == 64 else 'w'), 'd' if vsew == 64 else 'w',rs2,  n, 'd' if vsew == 64 else 'w',rd, rs2, n), file=f)
        for n in range(1, 32):
            if n % lmul != 0:
                continue
            rs1, rs2 = valid_aligned_regs(n)
            ...
            TEST_CASE_FP( testnum, v%d, flags, result, val1, val2, \\\n\
                ...
                fl%s f2, 8(a0); \\\n\
                inst v%d, v%d, v%d; \\\n\
            )" % (n, n, 'd' if vsew == 64 else 'w','d' if vsew == 64 else 'w',rs2, rs1,'d' if vsew == 64 else 'w', n, rs2, rs1), file=f)
    if test_vf:
        for n in range(1,32):
            if n == 2 or n % lmul != 0:
                continue
            print("#define TEST_FPRED_VF_OP_rs1_%d( testnum, inst, flags, result, val1, val2 )"%n + " \\\n\
                TEST_CASE_FP( testnum, v24, flags, result, val1, val2,   \\\n\
                    fl%s f0, 0(a0); "%('d' if vsew == 64 else 'w') + "\\\n\
                    fl%s f%d, 4(a0);"%('d' if vsew == 64 else 'w') + " \\\n\
                    vfmv.s.f v8, f0; \\\n\
                    fl%s f2, 8(a0); "%('d' if vsew == 64 else 'w') + "\\\n\
                    inst v24, v8, f%d; "%(n,n) + " \\\n\
                )", file=f)
