...
def generate_tests(f, rs1_val, rs2_val):
    ...
    for i in range(len(rs1_val)):        
        print("TEST_FP_VV_OP( %d,  %s.vs, 0xff100,               5201314,        %s,        %s );"%(n, instr, rs1_val[i], rs2_val[i]), file=f)
        n +=1
    ...
    for i in range(len(rs1_val)):
        k = i%31+1
        print("  TEST_FP_VV_OP_rd_%d( "%k+str(n)+",  %s.vs, 0xff100, "%instr+"5201314"+", "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
        n+=1
        
        k = i%31+1
        print("  TEST_FP_VV_OP_rs1_%d( "%k+str(n)+",  %s.vs, 0xff100, "%instr+"5201314"+", "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
        n+=1
...
def create_empty_test_vfredmin(xlen, vlen, vsew, lmul, vta, vma, output_dir):
...
def create_first_test_vfredmin(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
