...
def generate_tests(f, rs1_val, rs2_val):    
    ...
    for i in range(len(rs1_val)):        
        print("TEST_FP_HEX_1OPERAND_OP( %d,  %s7.v, 0xff100,               5201314,        %s );"%(n, instr, rs1_val[i]), file=f)
        n += 1
    ...
    for i in range(len(rs1_val)):     
        ...
        print("  TEST_FP_HEX_1OPERAND_OP_rd_%d( "%k+str(n)+",  %s7.v, 0xff100, "%instr +"5201314"+ ", " +rs1_val[i]+ " );",file=f)
        n += 1
        ...
        print("  TEST_FP_HEX_1OPERAND_OP_rs1_%d( "%k+str(n)+",  %s7.v, 0xff100, "%instr +"5201314"+ ", " +rs1_val[i]+ " );",file=f)
        n += 1
...
def create_empty_test_vfrec(xlen, vlen, vsew, lmul, vta, vma, output_dir):
...
def create_first_test_vfrec(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
