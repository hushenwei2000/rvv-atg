...
def generate_macros(f):
    ...
    for n in range(4,32,2):
        print("#define TEST_W_FP_VV_OP_rd%d( testnum, inst, finst, flags, val1, val2 )"%n + " \\\n\
...
def generate_tests(f):
    ...
    for i in range(len(rs1_val)):
        n += 1
        print("  TEST_W_FP_VV_OP( "+str(n)+",  %s.vv, fsub.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    ...
    for i in range(len(rs1_val)):
        n += 1
        print("  TEST_W_FP_VF_OP( "+str(n)+",  %s.vf, fsub.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    ...
    for i in range(len(rs1_val)):
        n += 1
        print("  TEST_W_FP_WV_OP( "+str(n)+",  %s.wv, fsub.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    ...
    for i in range(len(rs1_val)):
        n += 1
        print("  TEST_W_FP_WF_OP( "+str(n)+",  %s.wf, fsub.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    ...
    for i in range(len(rs1_val)):     
        k = i % 15 + 1
        n += 1
        print("  TEST_W_FP_VV_OP_rd%d( "%(k*2)+str(n)+",  %s.vv, fsub.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
        
        k = i % 30 + 2
        if (k == 14):
            continue;
        n += 1
        print("  TEST_W_FP_VV_OP_1%d( "%k+str(n)+",  %s.vv, fsub.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
...
def create_empty_test_vfwsub_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_W_FP_VV_OP( 1,  %s.vv, fsub.d, 0, 1, 1);"%instr, file=f)
...
def create_first_test_vfwsub_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
