...
def generate_macros(f):
    for n in range(2,32):
        print("#define TEST_FP_HEX_1OPERAND_OP_2%d( testnum, inst, flags, result, val )"%n + " \\\n\
    ...
    for n in range(2,32):
        print("#define TEST_FP_HEX_1OPERAND_OP_rd%d( testnum, inst, flags, result, val )"%n + " \\\n\
    ...
    print("#define TEST_FP_HEX_1OPERAND_OP_rd1( testnum, inst, flags, result, val ) \\\n\
...
def generate_tests(f):
    ...
    for i in range(len(rs2_val)):     
        k = i % 31 + 1
        n += 1
        print("  TEST_FP_HEX_1OPERAND_OP_rd%d( "%k+str(n)+",  vfrsqrt7.v, 0xff100, 5201314, "+rs2_val[i]+" );",file=f)
        
        k = i % 30 + 2
        n += 1
        print("  TEST_FP_HEX_1OPERAND_OP_2%d( "%k+str(n)+",  vfrsqrt7.v, 0xff100, 5201314, "+rs2_val[i]+" );",file=f)
