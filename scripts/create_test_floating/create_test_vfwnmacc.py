...
def generate_macros(f):
    for n in range(2,32):
        print("#define TEST_W_FP_VV_OP_NEGRESULT_1%d( testnum, inst, finst, flags, val1, val2 )"%n + " \\\n\
...
def generate_tests(f):
    ...
    for i in range(len(rs1_val)):
        n += 1
        print("  TEST_W_FP_VV_OP_NEGRESULT( "+str(n)+",  %s.vv, fmul.d, 0xff100, "%instr+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    ...
    for i in range(len(rs1_val)):
        n += 1
        print("  TEST_W_FP_VF_OP_RV_NEGRESULT( "+str(n)+",  %s.vf, fmul.d, 0xff100, "%instr+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    ...
    for i in range(len(rs1_val)):     
        k = i % 15 + 1
        n += 1
        print("  TEST_W_FP_VV_OP_NEGRESULT_rd%d( "%(k*2)+str(n)+",  %s.vv, fmul.d, 0xff100, "%instr+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
        
        k = i % 30 + 2
        if (k == 14):
            continue;
        n += 1
        print("  TEST_W_FP_VV_OP_NEGRESULT_1%d( "%k+str(n)+",  %s.vv, fmul.d, 0xff100, "%instr+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
...
def create_empty_test_vfwnmacc_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir):
...
def create_first_test_vfwnmacc_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
