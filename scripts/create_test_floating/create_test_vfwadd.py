...
def generate_tests(f):
    ...
    for i in range(len(rs1_val)):     
        k = i % 15 + 1
        n += 1
        print("  TEST_W_FP_VV_OP_rd%d( "%(k*2)+str(n)+",  %s.vv, fadd.d, "%instr+"0xff100, "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
...
def create_empty_test_vfwadd_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_W_FP_VV_OP( 1,  %s.vv, fadd.d, 0, 1, 1);"%instr, file=f)
...
def create_first_test_vfwadd_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
