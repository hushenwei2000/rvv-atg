...
def generate_tests(f, lmul):
    ...
    for i in range(len(rs2_val)):
        ...
        n += 1
        print("  TEST_W_FP_VV_OP_rd%d( "%k+str(n)+",  %s.vv, fmul.d, "%instr+"0xff100, "+rs1_val[i]+", "+rs2_val[i]+" );",file=f)
...
def create_empty_test_vfwmsac_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir):
...
def create_first_test_vfwmsac_b1(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
