...
def generate_macros(f):
    for n in range(1, 32):
        print("#define TEST_AVG_VV_OP_1%d( testnum, inst, result00, result01, result10, result11, val2, val1 ) "%n + " \\\n\
...
def generate_tests(f, rs1_val, rs2_val):
    ...
    for i in range(100):     
        k = i%31+1
        n+=1
        print("  TEST_AVG_VV_OP_rd%d( "%k+str(n)+",  %s.vv, "%instr+"0x5201314"+", "+"0x5201314"+", "+"0x5201314"+", "+"0x5201314"+", "+rs2_val[i]+", "+rs1_val[i]+");",file=f)
        
        k = i%30+2
        # if(k==14):
        #     continue;
        n +=1
        print("  TEST_AVG_VV_OP_1%d( "%k+str(n)+",  %s.vv, "%instr+"0x5201314"+", "+"0x5201314"+", "+"0x5201314"+", "+"0x5201314"+", "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    print("  #-------------------------------------------------------------", file=f)
...
def create_empty_test_vasubu(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_AVG_VV_OP( 1,  vasubu.vv, 0x0, 0x0, 0x0000, 0x000, 0x00, 0x00 );", file=f)
...
def create_first_test_vasubu(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
