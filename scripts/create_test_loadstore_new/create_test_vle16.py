    ...
    for i in range(1):
        ...
        for n in range(1,32):
            if emul <= 8 and n + emul <= 32 and n % emul == 0:
                print("  TEST_LOAD_V%dX7_offset( " %(n)+str(n)+",  vle16.v );", file=f)
        for n in range(1,32):
            if emul <= 8 and n + emul <= 32 and n % emul == 0:      
                print("  TEST_LOAD_V%dXFF_offset( " %(n)+str(n+32)+",  vle16ff.v );", file=f)
        for n in range(5,31):
            if emul <= 8 and 2 + emul <= 32 and 2 % emul == 0 and n != 7:
                print("  TEST_LOAD_V2X%d_offset( " %(n)+str(n+64)+",  vle16.v );", file=f)
...
def create_empty_test_vle16(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    for i in range(1,32):
        generate_macros_load_vx_offset(f, 16, 0, i, 7); 
    for i in range(5,31):
        generate_macros_load_vx_offset(f, 16, 0, 2, i); 
    # Generate tests
    generate_tests(f, vsew, lmul)

    print_load_ending_new(f, 16, 2, is_vx = True)
