...
def generate_macros(f):
    ...
    print("#define TEST_VV_OP_rd1( testnum, inst, result, val2, val1 ) \\\n\
    ...
    print("#define TEST_VV_OP_rd2( testnum, inst, result, val2, val1 ) \\\n\
...
def generate_tests(f, rs1_val, rs2_val):
    ...
    for i in range(100):     
        k = i%31+1
        n+=1
        print("  TEST_VV_OP_rd%d( "%k+str(n)+",  %s.vv, "%instr+"0x5201314"+", "+rs1_val[i]+", "+rs2_val[i]+");",file=f)
        
        k = i%30+2
        # if(k==14):
        #     continue;
        n +=1
        print("  TEST_VV_OP_1%d( "%k+str(n)+",  %s.vv, "%instr+"0x5201314"+", "+rs1_val[i]+" , "+rs2_val[i]+");",file=f)
    print("  #-------------------------------------------------------------", file=f)
...
def create_empty_test_vdivu(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_VV_OP( 1, vdivu.vv, 2, 1, 1 );", file=f)
...
def create_first_test_vdivu(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
