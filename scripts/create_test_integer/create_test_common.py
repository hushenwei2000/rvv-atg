...
def generate_tests_vw(f, rs1_val, rs2_val, instr, lmul, generate_wvwx = True):
    ...
    for i in range(min(32, loop_num)):
        ...
        if k%(2*lmul)==0 and k != 8 and k != 16 and k != 24:
            n+=1
            print("  TEST_W_VV_OP_rd%d( "%k+str(n)+",  %s.vv, "%instr+"rd_data_vv+%d, rs2_data+%d, rs1_data+%d)"%(i*step_bytes_double, i*step_bytes, i*step_bytes),file=f)
        
        k = i%30+2
        if k % lmul == 0 and k != 16 and k != 8 and k != 24:
            n +=1
            print("  TEST_W_VV_OP_1%d( "%k+str(n)+",  %s.vv, "%instr+"rd_data_vv+%d, rs2_data+%d, rs1_data+%d)"%(i*step_bytes_double, i*step_bytes, i*step_bytes),file=f)
    ...
    if generate_wvwx:
        ...
        for i in range(loop_num):
            ...
            print("  TEST_W_WV_OP( "+str(n)+",  %s.wv, " %
                instr+"rd_data_wv+%d, rs2_data_widen+%d, rs1_data+%d)"%(i*step_bytes_double, i*step_bytes, i*step_bytes), file=f)
        print("  #-------------------------------------------------------------", file=f)
        ...
        for i in range(loop_num):
            ...
            print("  TEST_W_WX_OP( "+str(n)+",  %s.wx, " %
                instr+"rd_data_wx+%d, rs2_data_widen+%d, %s)"%(i*step_bytes_double, i*step_bytes, rs1_val[i]), file=f)
...
def generate_tests_vmadc(instr, f, rs1_val, rs2_val, lmul, generate_vi = True):
    ...
    for i in range(loop_num):
        ...
        print("  TEST_MADC_VX_OP( "+str(n)+",  %s.vx, " %
               instr+"5201314, rs2_data+%d, %s)"%(i*step_bytes, rs1_val[loop_num]), file=f)
    ...
    if generate_vx:
        ...
        for i in range(min(32, loop_num)):
            ...
            if k == 0 or k == 24 or k % (lmul * 2) != 0:
                continue
            n+=1
            print("  TEST_VXM_OP_rd%d( "%k+str(n)+",  %s.vx, "%instr+"5201314, rs2_data+%d, %s)"%(i*step_bytes, rs1_val[loop_num]),file=f)
            
            k = i%30+2
            if k == 0 or k == 8 or k == 16 or k == 24 or k % (lmul * 2) != 0:
                continue
            n +=1
            print("  TEST_VXM_OP_1%d( "%k+str(n)+",  %s.vx, "%instr+"5201314, rs2_data+%d, %s)"%(i*step_bytes, rs1_val[loop_num]),file=f)
