...
def generate_tests(f, rs1_val, rs2_val):
    ...
    for i in range(100):     
        ...
        n+=1
        if k%2==0:
            print("  TEST_W_VV_OP_WITH_INIT_rd%d( "%k+str(n)+",  %s.vv, "%instr+"0x5201314"+", "+rs2_val[i]+", "+rs1_val[i]+");",file=f)
        
        k = i%30+2
        # if(k==14):
        #     continue;
        n +=1
        print("  TEST_W_VV_OP_WITH_INIT_1%d( "%k+str(n)+",  %s.vv, "%instr+"0x5201314"+", "+rs2_val[i]+", "+rs1_val[i]+" );",file=f)
    print("  #-------------------------------------------------------------", file=f)
...
def create_empty_test_vwmaccsu(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_W_VX_OP_RV( 1, vwmaccsu.vx, 2, 1, 1 );", file=f)
...
def create_first_test_vwmaccsu(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
