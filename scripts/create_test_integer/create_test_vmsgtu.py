...
def generate_tests(f, rs1_val, rs2_val):
    ...
    for i in range(100):     
        k = i%31+1
        n+=1
        print("  TEST_VXM_OP_rd%d( "%k+str(n)+",  %s.vx, "%instr+"0x5201314"+", "+rs1_val[i]+", "+rs2_val[i]+");",file=f)
        
        k = i%30+2
        # if(k==14):
        #     continue;
        n +=1
        print("  TEST_VXM_OP_1%d( "%k+str(n)+",  %s.vx, "%instr+"0x5201314"+", "+rs1_val[i]+" , "+rs2_val[i]+");",file=f)
    print("  #-------------------------------------------------------------", file=f)
...
def create_empty_test_vmsgtu(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_VXM_OP( 1, vmsgtu.vx, 2, 1, 1 );", file=f)
...
def create_first_test_vmsgtu(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
