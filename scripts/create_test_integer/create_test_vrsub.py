...
def generate_tests(f, rs1_val, rs2_val):
    ...
    for i in range(100):     
        k = i%31+1 
        n+=1
        print("  TEST_VX_OP_rd%d( "%k+str(n)+",  %s.vx, "%instr+"0x5201314"+", "+rs1_val[i]+", "+rs2_val[i]+");",file=f)
        n+=1
        ...
        #     continue;
        n +=1
        print("  TEST_VX_OP_1%d( "%k+str(n)+",  %s.vx, "%instr+"0x5201314"+", "+rs1_val[i]+", "+rs2_val[i]+");",file=f)
        print("  #-------------------------------------------------------------", file=f)
...
def create_empty_test_vrsub(xlen, vlen, vsew, lmul, vta, vma, output_dir):
    ...
    print("  TEST_VX_OP( 1, vrsub.vx, 2, 1, 1 );", file=f)
